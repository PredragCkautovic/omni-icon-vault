...
class IconStore:
    ...
    def reload(self,force=False):
        ...
        self.stats={'total':len(data),'sourceCount':len(self.counts),'kinds':dict(kinds),'formats':dict(formats),'styles':dict(styles),'sources':self.counts}
    ...
    def _filter(self,source='all',fmt='all'):
    ...
    def search(self,q='',source='all',fmt='all',sort='relevance',offset=0,limit=80):
        limit=max(1,min(int(limit),500));offset=max(0,int(offset));xs=self._filter(source,fmt)
        if q.strip():
    ...
    def random(self,source='all',fmt='all'):
        xs=self._filter(source,fmt);return random.choice(xs) if xs else None
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            if path=='/api/health':
                store.reload();return self.json_response({'ok':True,'version':version(),'icons':len(store.items),'sources':store.counts,'sourceCount':len(store.counts)})
            if path=='/api/stats':
                store.reload();return self.json_response({'ok':True,**store.stats})
            if path=='/api/search':
                q=qs.get('q',[''])[0];source=qs.get('source',['all'])[0];fmt=qs.get('format',['all'])[0];sort=qs.get('sort',['relevance'])[0]
                offset=self.qint(qs,'offset',0);limit=self.qint(qs,'limit',80);preview=qs.get('include',[''])[0]=='preview'
                xs,total=store.search(q,source,fmt,sort,offset,limit)
                return self.json_response({'ok':True,'query':q,'source':source,'format':fmt,'sort':sort,'offset':offset,'count':len(xs),'total':total,'items':[store.summary(i,preview) for i in xs]})
            if path=='/api/icon':
            ...
            if path=='/api/random':
                source=qs.get('source',['all'])[0];fmt=qs.get('format',['all'])[0];i=store.random(source,fmt)
