...
function supportsCopy(i,mode){if(!i||!mode||mode==='smart'||mode==='all')return true;if(Array.isArray(i.capabilities))return i.capabilities.includes(mode);if(mode==='id'||mode==='manifest')return !!key(i);if(mode==='svg')return !!i.svg;if(mode==='glyph')return !!i.char;if(mode==='html')return !!i.html;if(mode==='css')return !!i.css;if(mode==='react')return !!i.svg;if(mode==='asset')return !!i.raster;return false}
function copyModeLabel(mode){return ({svg:'SVG',glyph:'Glyph',html:'HTML',css:'CSS',react:'React JSX',id:'ID',manifest:'Manifest'})[mode]||''}
function capabilityList(i){const xs=Array.isArray(i?.capabilities)?i.capabilities.filter(x=>['svg','glyph','html','css','react'].includes(x)):['svg','glyph','html','css','react'].filter(x=>supportsCopy(i,x));return [...new Set(xs)]}
...
function reactValue(i){
  if(!i?.svg)return'';
  const name=`Icon${pascal(i.name||i.label||'Omni')}`;
  let svg=String(i.svg).replace(/class=/g,'className=').replace(/fill-rule=/g,'fillRule=').replace(/clip-rule=/g,'clipRule=').replace(/stroke-width=/g,'strokeWidth=').replace(/stroke-linecap=/g,'strokeLinecap=').replace(/stroke-linejoin=/g,'strokeLinejoin=');
  svg=svg.replace(/^<svg\b/,`<svg {...props}`);
  return `export function ${name}(props){\n  return (\n    ${svg.replace(/\n/g,'\n    ')}\n  );\n}`;
}
function value(i,mode){if(!i)return'';if(mode==='smart')mode=smart(i);if(mode==='id')return key(i);if(mode==='manifest')return manifestValue(i);if(mode==='svg')return i.svg||'';if(mode==='glyph')return i.char||'';if(mode==='html')return i.html||'';if(mode==='css')return i.css||'';if(mode==='react')return reactValue(i);if(mode==='asset')return i.raster||'';return''}
...
function updateUrl(){const u=new URL(location.href);state.query?u.searchParams.set('q',state.query):u.searchParams.delete('q');state.source!=='all'?u.searchParams.set('source',state.source):u.searchParams.delete('source');state.copyMode!=='smart'?u.searchParams.set('copy',state.copyMode):u.searchParams.delete('copy');state.current?u.searchParams.set('icon',key(state.current)):u.searchParams.delete('icon');history.replaceState(null,'',u)}
function initFromUrl(){const u=new URL(location.href);state.query=u.searchParams.get('q')||'';state.source=u.searchParams.get('source')||'all';const copy=u.searchParams.get('copy');if(['smart','svg','glyph','html','css','react','id','manifest'].includes(copy))state.copyMode=copy;state.deepLinkIcon=u.searchParams.get('icon')||'';$('#search').value=state.query}
...
const PREVIEW_SIZE_MIN=8,PREVIEW_SIZE_MAX=512,PREVIEW_SIZE_DEFAULT=96;
...
function runDiscovery(query){$('#search').value=query;state.query=query;search(true);$('#search').focus()}
function openShortcuts(){$('#shortcutModal').classList.remove('hidden')}
function closeShortcuts(){$('#shortcutModal').classList.add('hidden')}
async function copyDeepLink(){if(!state.current)return;updateUrl();await copy(location.href,'Copied icon deep link')}
function applyPresetSize(size){$('#previewSize').value=String(size);$('#previewSizeInput').value=String(size);applyPreviewControls('range')}
...
document.addEventListener('click',e=>{const q=e.target.closest('[data-query]');if(q)runDiscovery(q.dataset.query)});
$('#shortcutButton').onclick=openShortcuts;$('#closeShortcutModal').onclick=closeShortcuts;$('#shortcutModal').onclick=e=>{if(e.target===$('#shortcutModal'))closeShortcuts()};
$('#sizePresets').onclick=e=>{const b=e.target.closest('[data-size]');if(b)applyPresetSize(Number(b.dataset.size))};
$('#shareButton').onclick=copyDeepLink;
...
document.addEventListener('keydown',e=>{const typing=['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName);if(e.key==='/'&&!typing){e.preventDefault();$('#search').focus();$('#search').select()}if(e.key==='?'&&!typing){e.preventDefault();openShortcuts()}if(e.key.toLowerCase()==='r'&&!typing){e.preventDefault();$('#randomIcon').click()}if(e.key.toLowerCase()==='f'&&!typing&&state.current){e.preventDefault();toggleFavorite(state.current)}if(e.key==='Escape'){if(!$('#shortcutModal').classList.contains('hidden'))closeShortcuts();else if($('#details').classList.contains('open'))closeDetails();else closeMobileSidebar()}if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();$('#search').focus();$('#search').select()}});

async function boot(){initFromUrl();restorePreviewSize();setTheme(state.theme);$('#sortMode').value=['relevance','name','pack'].includes(state.sort)?state.sort:'relevance';$('#copyMode').value=['smart','svg','glyph','html','css','react','id','manifest'].includes(state.copyMode)?state.copyMode:'smart';syncCapabilityUi();applyDensity();const ok=await loadBootstrap();if(!ok)return;syncActiveNav();await search(true);if(state.deepLinkIcon){try{const d=await fetchJson(`${API}/api/icon?id=${encodeURIComponent(state.deepLinkIcon)}`);await openDetails(d.icon)}catch{toast('Linked icon is not available locally')}}}
boot();
})();
