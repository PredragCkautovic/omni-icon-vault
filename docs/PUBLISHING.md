...
git init
git add .
git commit -m "Omni Icon Vault 4.1.0"
git branch -M main
```
...
## Publish v4.1.0 — recommended automated path
...
```bash
git tag -a v4.1.0 -m "Omni Icon Vault 4.1.0" 2>/dev/null || true
git push origin v4.1.0
```
...
gh release create v4.1.0 \
  dist/Omni-Icon-Vault-4.1.0-source.zip \
  dist/Omni-Icon-Vault-4.1.0-windows.zip \
  dist/Omni-Icon-Vault-4.1.0-macos.zip \
  dist/Omni-Icon-Vault-4.1.0-linux.zip \
  dist/SHA256SUMS.txt \
  --verify-tag \
  --title "Omni Icon Vault 4.1.0" \
...
```bash
gh release upload v4.1.0 dist/Omni-Icon-Vault-4.1.0-*.zip dist/SHA256SUMS.txt --clobber
```
